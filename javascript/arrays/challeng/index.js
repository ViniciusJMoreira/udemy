...
dogsJulia.splice(-2);
// dogsJulia.forEach(checkDogs);
// dogsKate.forEach(checkDogs);


const checkDogs2 = function(dogsJuly, dogsKate) {
  const dogsJulyCorr = dogsJuly.slice(0);
  dogsJulyCorr.splice(0,1);
  dogsJulyCorr.splice(-2);
  const dogs = dogsJulyCorr.concat(dogsKate);
  dogs.forEach((age,i) => {
    if (age > 3) {
      console.log(`Dog number ${i + 1} is an adult and is ${age} years old`);
    } else {
